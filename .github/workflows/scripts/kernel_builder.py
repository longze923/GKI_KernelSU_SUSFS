...
"""
    ...
    def _bbg_generate_flask_headers(self, common_dir):
        """BBG 需要安全/selinux/flask.h 与 av_permissions.h，但 GKI 的 LLVM 工具链
        无法编译 BBG 自带的 genheaders（缺少 ld）。这里按 ABK 的兼容修复直接从
        classmap.h/initial_sid_to_string.h 生成，并让 BBG Makefile 优先使用
        srctree 下预生成的头文件。"""
        selinux_dir = common_dir / "security/selinux"
        include_dir = selinux_dir / "include"
        classmap_h = include_dir / "classmap.h"
        initial_sid_h = include_dir / "initial_sid_to_string.h"
        if not (classmap_h.exists() and initial_sid_h.exists()):
            logger.warning("BBG: selinux classmap headers missing, skip flask generation")
            return
        ...
        if not body_match:
            raise RuntimeError("BBG: secclass_map not found")
        secclasses = []
        ...
        if not sid_match:
            raise RuntimeError("BBG: initial_sid_to_string not found")
        initial_sids = []
        ...
        for class_name, perms in secclasses:
            for bit, perm in enumerate(perms):
                if bit >= 32:
                    raise RuntimeError(f"BBG: too many permissions for {class_name}")
        ...
        logger.info("BBG: flask.h/av_permissions.h 已预生成")

        makefile = common_dir / "Baseband-guard/Makefile"
        if makefile.exists():
            ...
            logger.info("BBG: Makefile 已允许使用 srctree 预生成 flask.h")
